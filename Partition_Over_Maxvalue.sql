...
-- Under Anonymous Block --
DECLARE
    l_table_name           VARCHAR2(30) := 'split_partition';
    l_table_owner          VARCHAR2(30) := 'hr';
    l_identify_name_cur    sys_refcursor;
    l_old_partition_name   VARCHAR2(50);
    l_high_value           VARCHAR2(30);
    l_flag                 VARCHAR2(10);
    ...
    l_high_value_3         VARCHAR2(50);
    flag_o                 VARCHAR2(50);
    high_value_final_o     VARCHAR2(50);
    old_partition_name_o   VARCHAR2(50);
BEGIN
    -- To segregate the partition value in three parts
    pk_partition_over_maxvalue.sp_identify_partition_name(l_table_name,l_table_owner,l_identify_name_cur);
    LOOP
      FETCH l_identify_name_cur INTO l_old_partition_name,l_high_value,l_flag;
      EXIT WHEN l_identify_name_cur%NOTFOUND;
      IF (l_old_partition_name IS NOT NULL) THEN
    ...
    dbms_output.put_line('Flagging Mechanism to identify the max Partition Value and Name => '||l_flag||' , '||high_value_final_o||' , '||old_partition_name_o);
    Dbms_Output.Put_Line('*********************************************************************************************************************************'); 
    ...
    END;
    Dbms_Output.Put_Line('*********************************************************************************************************************************');
    dbms_output.put_line('Flagging Mechanism to Identify the Nature of Partition => '||flag_o||' , '||high_value_final_o||' , '||old_partition_name_o);
    Dbms_Output.Put_Line('*********************************************************************************************************************************');
END ;
/
...
/*
*********************************************************************************************************************************
Flagging Mechanism to identify the max Partition Value and Name => 4 , 120180107000 , P_097                                      
*********************************************************************************************************************************
*********************************************************************************************************************************
Flagging Mechanism to Identify the Nature of Partition          => 35 , 120180107000 , P_097                                              
*********************************************************************************************************************************                     
*/
...
DECLARE
    l_table_name           VARCHAR2(30) := 'split_partition';
    l_table_owner          VARCHAR2(30) := 'hr';
    l_column_name_year     VARCHAR2(10) := '2018';
    l_old_partition_name   VARCHAR2(50);
    l_new_partition_name   VARCHAR2(50);
    l_high_value           VARCHAR2(30);
    l_flag                 VARCHAR2(10);
...
BEGIN
      ...
      BEGIN
           IF (l_flag <> '~') THEN 
              -- To find existing type of partition in Our Database
              IF l_flag >= 28 THEN 
                 l_column := 'TO_CHAR(''1''||a.column_name_1||''000'') ';
                 l_table_name_lookup := 'table_name_1';
              ELSIF l_flag = 7 THEN 
                 l_column := 'TO_CHAR(''1''||a.column_name_2||''000'') ';
                 l_table_name_lookup := 'table_name_2';
              ELSIF l_flag IN (4,5) THEN
                 l_column := 'TO_CHAR(MIN(a.column_name_3)) ';
                 l_table_name_lookup := 'table_name_2';
                 l_operator := 'HAVING ';
                 l_group_by := 'GROUP BY a.column_name_4 ';
              ELSIF l_flag = 1 THEN
                 l_column := 'TRIM(TO_CHAR(a.column_name_3)) ';
                 l_table_name_lookup := 'table_name_2';
              END IF;
                         ...
                         FROM (
                               ...
                               FROM (
                                     ...
                                     FROM 
                                          '||l_table_name_lookup||' a
                                     WHERE 
                                          TRIM(a.column_name_year) = '''||l_column_name_year||'''
                                     '||l_operator||' '||l_column||' > '''||l_high_value||'''
                                     AND NOT EXISTS (
                                                     SELECT
                                                          1
...
/*                                       
ALTER TABLE hr.split_partition SPLIT PARTITION P_097 AT (120180128000)
INTO 
(
 PARTITION P_098, 
 PARTITION P_097
);
ALTER TABLE hr.split_partition SPLIT PARTITION P_098 AT (120180225000)
INTO 
(
 PARTITION P_099, 
 PARTITION P_098
);
ALTER TABLE hr.split_partition SPLIT PARTITION P_099 AT (120180401000)
INTO 
(
 PARTITION P_100, 
 PARTITION P_099
);
ALTER TABLE hr.split_partition SPLIT PARTITION P_100 AT (120180429000)
INTO 
(
 PARTITION P_101, 
 PARTITION P_100
);
ALTER TABLE hr.split_partition SPLIT PARTITION P_101 AT (120180527000)
INTO 
(
 PARTITION P_102, 
 PARTITION P_101
);
ALTER TABLE hr.split_partition SPLIT PARTITION P_102 AT (120180701000)
INTO 
(
 PARTITION P_103, 
 PARTITION P_102
);
ALTER TABLE hr.split_partition SPLIT PARTITION P_103 AT (120180729000)
INTO 
(
 PARTITION P_104, 
 PARTITION P_103
);
ALTER TABLE hr.split_partition SPLIT PARTITION P_104 AT (120180826000)
INTO 
(
 PARTITION P_105, 
 PARTITION P_104
);
ALTER TABLE hr.split_partition SPLIT PARTITION P_105 AT (120180930000)
INTO 
(
 PARTITION P_106, 
 PARTITION P_105
);
ALTER TABLE hr.split_partition SPLIT PARTITION P_106 AT (120181028000)
INTO 
(
 PARTITION P_107, 
 PARTITION P_106
);
ALTER TABLE hr.split_partition SPLIT PARTITION P_107 AT (120181125000)
INTO 
(
 PARTITION P_108, 
 PARTITION P_107
);
ALTER TABLE hr.split_partition SPLIT PARTITION P_108 AT (120181230000)
INTO 
(
 PARTITION P_109, 
 PARTITION P_108
