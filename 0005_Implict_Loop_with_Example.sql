...
Syntax :
		...
		In Example 4-10, the EXIT WHEN statement inside the basic LOOP statement transfers control to the end of the current loop when x is greater than 3.
		...
		*
		...
		4.1.7.4.The NULL statement only passes control to the next statement. Some languages refer to such an instruction as a no-op (no operation).
		...
		4.1.8.Reraising Current Exception with RAISE Statement
		In an exception handler, you can use the RAISE statement to"reraise" the exception being handled. Reraising the exception passes it
		to the enclosing block, which can handle it further. (If the enclosing block cannot handle the reraised exception, then the exception
		propagates—see "Exception Propagation".) When reraising the current exception, you need not specify an exception name.
		In Example 11-11, the handling of the exception starts in the inner block and finishes in the outer block. The outer block declares the
		exception, so the exception name exists in both blocks, and each block has an exception handler specifically for that exception. The inner
		block raises the exception, and its exception handler does the initial handling and then reraises the exception, passing it to the outer block
		for further handling.

		Example 11-11 Reraising an Exception
		DECLARE
		...
		BEGIN
			BEGIN
				...
				END IF;
			EXCEPTION WHEN salary_too_high THEN  -- start handling exception
				erroneous_salary := current_salary;
				dbms_output.put_line('Salary '||erroneous_salary||' is out of range.');
				dbms_output.put_line ('Maximum salary is '||max_salary||'.');
				RAISE;  -- reraise current exception (exception name is optional)
			END;
		EXCEPTION WHEN salary_too_high THEN    -- finish handling exception
...
BEGIN
    FOR i IN 1..1000
    LOOP
       Dbms_Output.Put_Line(i||' = '||Chr(i)); -- Asici code conversion
    END LOOP;
END;
/
...
BEGIN
    ...
    LOOP
       BEGIN
           l_sub_value := SubStr(l_string_i,i,1);
           IF (regexp_like(l_sub_value,'[[:digit:]]' ))THEN
               IF ( l_flag = 0) THEN
                  l_total_no := regexp_substr(l_string_i,'[[:digit:]]+',i);
                      Dbms_Output.Put_Line(l_total_no);
                  l_total_sum := l_total_sum + l_total_no;
                  l_flag := 1;
               END IF;
            ELSIF (regexp_like(l_sub_value,'^[A-Z]$','i')) THEN
                l_flag := 0;
...
--Connect to SYS User
CREATE OR REPLACE DIRECTORY DIR_NAME AS  'C:\Users\suman\Desktop\OracleSQL\Seat plan';
...
BEGIN
     ...
     FOR i IN (SELECT ROWNUM rn,beit, civil, elex, comp  FROM student_list)
     LOOP
        IF (Mod(i.rn,2) = 0) THEN
            Dbms_Output.Put_Line(i.beit || '   '||i.civil ||'   '||'   '||i.elex||'   '||'   '||i.comp);
            EXECUTE IMMEDIATE 'INSERT INTO seat_plan VALUES ('''||i.beit||''', '''||i.civil||''','''|| i.comp||''','''|| i.elex||''')';
			COMMIT;
        ELSE
            Dbms_Output.Put_Line(i.civil || '   '||i.beit  ||'   '||'   '||i.comp||'   '||'   '||i.elex  );
            EXECUTE IMMEDIATE 'INSERT INTO seat_plan VALUES ('''||i.civil||''', '''||i.beit||''','''|| i.elex||''','''|| i.comp||''')';
			COMMIT;
        END IF;
     END LOOP;
END;
...
BEGIN
    LOOP
       ...
       EXIT WHEN (i > 3 );
    END LOOP;
          Dbms_Output.Put_Line('after loop : '||i);
END;
/
...
BEGIN
    ...
    CONTINUE;
    END IF;
    Dbms_Output.Put_Line('inside loop after continue: '||i);
    EXIT WHEN i = 5;
    END LOOP;
    Dbms_Output.Put_Line('outside loop: '||i);
END;
/
--CONTINUE WHEN
DECLARE
...
/
 ...
 --RAISE, AND HANDLING USER DEFINED EXCEPTION
...
BEGIN
    ...
    BEGIN
        sys_date := SYSDATE-3;
        IF (sys_date > SYSDATE - 2) THEN
           RAISE past_due;
        ELSE
            Dbms_Output.Put_Line(sys_date);
        END IF;
    END;
    EXCEPTION WHEN past_due THEN
      Dbms_Output.Put_Line('sys_date is more than expected');
END;
/
...
BEGIN
    ...
    EXCEPTION WHEN Invalid_Number THEN
     Dbms_Output.Put_Line('insert in different table');
END;
/